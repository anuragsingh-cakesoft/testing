...
export async function ProductHuntCommentExtract(page: Page, producthunt: { productHuntLaunchUrl: string, numberOfComments: number }): Promise<void> {
    try {
        ...
        await fs.writeFile('./03_ProductHuntCommentExtract/outputs/initial_reviews.json', JSON.stringify(allReviews, null, 2));

        // Calculate number of "Show More" clicks needed (10 reviews per page)
        const clicksNeeded = Math.ceil((totalReviews >= producthunt.numberOfComments ? producthunt.numberOfComments - 10 : totalReviews - 10) / 10);
        ...
        await fs.writeFile('./03_ProductHuntCommentExtract/outputs/producthunt_reviews.json', JSON.stringify(finalObject, null, 2));
