...
export async function getYouTubeComments(page: Page, videoUrl: string): Promise<void> {
    let allComments: any[] = [];
...
// } 
    ...
    if (allComments.length > 0) {
        try {
            const structuredComments = processYouTubeComments(allComments);
            const finalObject = {
            ...
            await Promise.all([
                fs.writeFile('./finalobject.json', JSON.stringify(finalObject, null, 2), 'utf-8'),
                fs.writeFile('./all_comments.json', JSON.stringify(allComments, null, 2), 'utf-8'),
                fs.writeFile('./structured_comments.json', JSON.stringify(structuredComments, null, 2), 'utf-8')
            ]);
